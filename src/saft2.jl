...
function Mix(componentes::Vector{String},MW::typeof([1.0]u"g/mol"),x::typeof([1.0]),y::typeof([1.0]),m::typeof([1.0]),σ::typeof([1.0]u"Å"),ϵ::typeof([1.0]u"K"),σᵢⱼ::typeof([1.0 1.0;1.0 1.0]u"Å"),ϵᵢⱼ::typeof([1.0 1.0;1.0 1.0]u"K"),k::Array{Float64,2})
    n = length(x) 
    Mix(n,componentes,MW,x,y,m,σ,ϵ,σᵢⱼ,ϵᵢⱼ,k)
end
...
#=function hidrocarburos(mix::mix)
    MW = mix.MW
...
@timeit to function zdisp(T,mix,ρ,phase)
    n = mix.n
...
@timeit to function z_hc(T,mix,ρ,phase)
    n = mix.n
    m,σ,ϵ,k = mix.m,mix.σ,mix.ϵ,mix.k
    phase == "gas" ? x = mix.y : x = mix.x
    T = T |> u"K"
...
@timeit to function obj_saft(η,T,P,mix,phase)
    n = mix.n
    m,σ,ϵ,k = mix.m,mix.σ,mix.ϵ,mix.k
    phase == "gas" ? x = mix.y : x = mix.x
    T = T |> u"K"
...
@timeit to function compresivilidad(η,T,mix,phase)
    n = mix.n
    m,σ,ϵ,k = mix.m,mix.σ,mix.ϵ,mix.k
    phase == "gas" ? x = mix.y : x = mix.x
    T = T |> u"K"
...
@timeit to function compr(T,P,mix,phase)
    n = mix.n
    m,σ,ϵ,k = mix.m,mix.σ,mix.ϵ,mix.k
...
@timeit to function fugF(T,P,mix,phase)
    T = T |> u"K"
...
@timeit to function mu_hc(T,mix,ρ,phase)
    n = mix.n
    m,σ,ϵ,k = mix.m,mix.σ,mix.ϵ,mix.k
    phase == "gas" ? x = mix.y : x = mix.x
    T = T |> u"K"
...
@timeit to function HelmholtzDisp(T,mix,ρ,phase)
    n = mix.n
    m,σ,ϵ,k = mix.m,mix.σ,mix.ϵ,mix.k
    σᵢⱼ,ϵᵢⱼ = mix.σᵢⱼ,mix.ϵᵢⱼ
    phase == "gas" ? x = mix.y : x = mix.x
...
@timeit to function mu_disp(T,mix,ρ,phase)
    n = mix.n
    m,σ,ϵ,k = mix.m,mix.σ,mix.ϵ,mix.k
    σᵢⱼ,ϵᵢⱼ = mix.σᵢⱼ,mix.ϵᵢⱼ
    phase == "gas" ? x = mix.y : x = mix.x
...
@timeit to function obj_buble(T,P,mix)
    iter = 0
    error = 100
...
function pxy(T,P,mix::mix,xx;uni=u"Torr")
    ...
    mix.x = xx[:,1]
    mix.y = mix.x 
    p[1],y[:,1] = obj_buble(T,P,mix1)
    for i=2:m
        mix.x = xx[:,i]
        mix.y = y[:,i-1] 
        p[i],y[:,i] = obj_buble(T,p[i-1],mix1)
    end
...
mix1 = mix(["metano","butano"],[16.043,58.123]u"g/mol",[0.0,1.0],[0.0 0.022;0.022 0.0])
xxx,yyy,ppp = pxy(21u"°C",3u"bar",mix1,[0:0.5:1];uni="bar")
plot(xxx[1,:],ustrip(ppp))
plot!(yyy[1,:],ustrip(ppp))

plot([1,2,3,5],[150,20,305,50])
plot([1,2,NaN,5,6],[150,20,5,50,520])
nothing

to
reset_timer!(to)

mix1 = mix(2,["metano","butano"],[16.043,58.123]u"g/mol",[0.5,0.5],[0.5,0.5],[.1,.11],[.1,.9]u"Å",[.1,.9]u"K",[0.0 0.022;0.022 0.0])
mix1.m,mix1.σ,mix1.ϵ = hidrocarburos(mix1)
mix1

mix2 = mix(["metano","hetano","propano"],[16.043,30.07,44.096]u"g/mol",[.1,.3,.6],[0 3.00e-4 1.15e-2;3.00e-4 0 5.10e-3;1.15e-2 5.10e-3 0])
mix2.m,mix2.σ,mix2.ϵ = hidrocarburos(mix2)
mix2

zdisp(21u"°C",mix1,.01u"Å^-3")
z_hc(21u"°C",mix1,.01u"Å^-3")
obj_saft(0.1379369490536057,21u"°C",100u"bar",mix1)
compr(21u"°C",100u"bar",mix1,"gas")
compr(21u"°C",100u"bar",mix1,"liq")
fugF(21u"°C",100u"bar",mix1,"gas")
fugF(21u"°C",100u"bar",mix1,"liq")
obj_buble(21u"°C",7u"bar",mix1)
...
zdisp(233.15u"K",mix2,1.19e-2u"Å^-3","liq")
zdisp(233.15u"K",mix2,1.19e-2u"Å^-3","gas")

z_hc(233.15u"K",mix2,1.19e-2u"Å^-3","liq")
z_hc(233.15u"K",mix2,1.19e-2u"Å^-3","gas")

mu_hc(233.15u"K",mix2,1.19e-2u"Å^-3","liq")
mu_hc(233.15u"K",mix2,1.19e-2u"Å^-3","gas")

obj_saft(0.1379369490536057,21u"°C",100u"bar",mix2)
compr(233.15u"K",1e5u"Pa",mix2,"gas")
compr(233.15u"K",1e5u"Pa",mix2,"liq")
fugF(233.15u"K",1e5u"Pa",mix2,"gas")
fugF(233.15u"K",1e5u"Pa",mix2,"liq")
HelmholtzDisp(233.15u"K",mix2,8.58e-3u"Å^-3","liq")
HelmholtzDisp(233.15u"K",mix2,3.13e-5u"Å^-3","gas")



obj_buble(21u"°C",50u"bar",mix1)

p1,mx3 = obj_buble(233.15u"K",1e5u"Pa",mix2)
fugF(233.15u"K",p1,mx3,"gas").*mx3.y-fugF(233.15u"K",p1,mx3,"liq").*mx3.x
